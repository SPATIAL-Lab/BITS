model {
  ...
  #generate time series of Re values
  for(i in 1:t){
    #weighted mean calculated by Sr concentration and ratio
    Re.mean[i] = (Sr.pel.m * w.pel.m * pel.ratio + Sr.hay.m * w.hay.m * hay.ratio)/(w.pel.m * pel.ratio + w.hay.m * hay.ratio)
  }
  ...
  #food ratio: hay is estimated to be ~ 70% of elephant diet
  hay.ratio = 1 - pel.ratio
  
  #pellet is estimated to be ~ 30% of elephant diet
  pel.ratio ~ dbeta(alpha, beta)
  
  alpha = 8
  
  beta = 16
  
  #Sr concentration of food as weights
  #1 pellets
  w.pel.m ~ dnorm(conc.pel.mean, 1/conc.pel.sd^2)
  #2 hay
  w.hay.m ~ dnorm(conc.hay.mean, 1/conc.hay.sd^2)
  
  #Sr 87/86 of food
  #1 pellets
  Sr.pel.m ~ dnorm(Sr.pel.mean, 1/Sr.pel.sd^2)
  #2 hay
  Sr.hay.m ~ dnorm(Sr.hay.mean, 1/Sr.hay.sd^2)
