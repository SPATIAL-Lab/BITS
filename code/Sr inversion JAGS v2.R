model {
  ...
  Rb.cal[1] ~ dnorm(R0, Sr.pre.b) #use a different error term
  Rs.cal[1] ~ dnorm(R0, Sr.pre.s)
  ...
  for (i in 1:t.cal){
    Rin.cal[i] ~ dnorm(Rin.mean[i], Sr.pre.s)# not measurement error here! use a different error term
  }
